...
type CartItem = { idProduct: number; isFoil: boolean; qty: number };
...
export default function BuylistPage() {
  ...
  const rows = useMemo(() => {
    ...
    return Array.from(merged.values()).map((it) => {
      ...
      const serverUnit = q?.unit ?? 0;
      const unit = round2(serverUnit * factor);
      const lineTotal = round2(unit * it.qty);
      return {
        ...it,
        serverUnit,
        unit,
  ...
  async function fetchQuotes() {
    ...
    try {
      ...
      const res = await fetch(`/api/prices/quote?pct=${percent}`, {
        method: "POST",
    ...
    try {
      ...
      const res = await fetch("/api/cart/submit", {
        ...
        body: JSON.stringify({
          email,
          items: rows.map((r) => ({ idProduct: r.idProduct, isFoil: r.isFoil, qty: r.qty })),
          meta: { percent, clientTotal: total },
        }),
      });
      const data = await res.json();
        ...
        <section className="grid gap-3 rounded-2xl border p-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <label className="text-sm opacity-70">Uitbetaal-%</label>
              <input
                type="range"
                min={30}
                max={90}
