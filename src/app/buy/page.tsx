        ...
        <div className="sticky top-16 z-10 rounded-xl af-panel border p-3">
          <div className="grid gap-3 md:grid-cols-[1fr_auto_auto] items-center">
            ...
            <div className="flex gap-2">
              <Button variant={view === "grid" ? "default" : "secondary"}
                      onClick={() => setView("grid")}>Grid</Button>
              <Button variant={view === "list" ? "default" : "secondary"}
...
<div className="relative w-full overflow-hidden rounded-lg border af-panel h-[200px] sm:h-[220px]">
  {(it.imageNormal || it.imageSmall) ? (
    ...
    <div className="flex h-full items-center justify-center af-muted text-xs">No image</div>
  )}
</div>
...
{view === "list" && (
  <div
    className="mt-8 grid gap-6 grid-cols-[minmax(0,1fr)_380px] items-start" // <- was `lg:grid-cols...`
  >
    {/* LEFT: list */}
      ...
      {visible.map((it) => {
        ...
        return (
          <div
            ...
            onMouseEnter={() => {
  console.log("[hover]", it.id, it.name, it.imageSmall || it.imageNormal);
  setPreview(it);
}}
            tabIndex={0}
          >
            {/* tiny thumb (explicit W/H; no fill) */}
            <div
              className={[
            ...
            >
              {(it.imageSmall || it.imageNormal) ? (
                <Image
                  ...
                  alt={it.name}
                  width={36}   // explicit size => no zero-height
                  height={48}
...
<div className="hidden sm:block text-[10px] af-muted text-right mr-1">
  {getPref(it.id).condition}{getPref(it.id).foil ? " • Foil" : ""}
</div>

  {/* condition select (same options as preview) */}
  <select
    value={getPref(it.id).condition}
    onChange={(e) => setPrefCond(it.id, e.target.value as Condition)}
    className="h-7 rounded border border-[var(--border)] bg-[var(--bg2)] px-2 text-xs af-text"
    title="Conditie"
    onClick={(e) => e.stopPropagation()}     // prevent hover flicker
    onMouseDown={(e) => e.stopPropagation()} // keep preview steady
  >
    <option value="NM">NM</option>
    <option value="EX">EX</option>
    <option value="GD">GD</option>
  ...
  <button
    type="button"
    onClick={(e) => { e.stopPropagation(); togglePrefFoil(it.id); }}
    className={[
