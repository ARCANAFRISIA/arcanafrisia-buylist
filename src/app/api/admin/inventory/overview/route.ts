// src/app/api/admin/inventory/overview/route.ts
...
const onlyPlayed = url.searchParams.get("onlyPlayed") === "1";
    ...
    // hoofdquery: balances + scryfall + eerste lot sourceCode
  ...
  LEFT JOIN "ScryfallLookup"   sf ON sf."cardmarketId" = b."cardmarketId"
  LEFT JOIN "CMPriceGuide"     pg ON pg."cardmarketId" = b."cardmarketId"
  LEFT JOIN (
    SELECT
      bm."cardmarketId",
  ...
  onlyPlayed     // $5
    ...
    // BigInt / Decimal naar gewone numbers voor JSON
const total = Number(totalRows[0]?.count || 0);

// BigInt / Decimal → Number voor JSON
const safeItems = items.map((row) => ({
  ...
  avgUnitCostEur:
    row.avgUnitCostEur == null ? null : Number(row.avgUnitCostEur),
