// src/app/api/providers/ctbulk/orders/sync/route.ts
...
// ✅ BULK token
const CT_TOKEN = process.env.CTBULK_TOKEN;

const SOURCE = "CTBULK" as const;

const centsToEur = (c?: number | null) => (c == null ? null : Number((c / 100).toFixed(2)));

function normalizeCTLanguage(raw?: string | null): string | null {
  if (!raw) return null;
...
async function ctFetch(path: string) {
  if (!CT_TOKEN) throw new Error("CTBULK_TOKEN missing");
  ...
  try {
    ...
    if (!res.ok) throw new Error(`CT ${res.status} ${path} :: ${bodyText}`);
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const maxPages = Math.max(1, Math.min(Number(url.searchParams.get("maxPages") ?? "1"), 10));
        ...
        for (const o of orders) {
          ...
          for (const li of items) {
            ...
            const languageRaw: string | null =
              ...
              (li.properties?.mtg_language as string | null) ??
              (li.properties?.mtg_lang as string | null) ??
              (li.language as string | null) ??
              null;
            ...
            await prisma.salesLog.upsert({
              where: (ctLineId != null && Number.isFinite(Number(ctLineId)))
                ? { source_ctLineId: { source: SOURCE, ctLineId: Number(ctLineId) } }
              ...
              update: {
                ...
                language: line.language ?? language ?? undefined,
                qty,
                unitPriceEur: unit,
                lineTotalEur: gross,
                feeEur: feeAlloc,
    ...
    return NextResponse.json(
      { ok: true, source: SOURCE, limit, page, states, processedOrders, linesProcessed, salesUpserts },
