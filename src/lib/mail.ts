...
export async function sendMail(args: SendMailArgs) {
  ...
  if (!to) {
    console.warn("[mail] skipped: no recipient (MAIL_ADMIN/MAIL_FROM missing and no 'to')");
    return { skipped: true };
  }
...
export function customerConfirmationHtml(args: {
  ...
  <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial">
    ...
    <p style="font-size:12px;color:#666;margin-top:16px">
      Let op: dit is een voorlopige berekening op basis van opgegeven conditie. Definitieve prijs na ontvangst & controle.
    </p>
  </div>`;
}
